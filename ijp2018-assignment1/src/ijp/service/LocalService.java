 ...
 * <li><code>LocalService.debug</code> - <code>true</code> or <code>false</code> to enable/disable debugging.</li>
 * <li><code>LocalService.path</code> - a colon-separated list of locations containing images.</li></ul>
...
public class LocalService implements Service {
	...
	private HashMap<String,URL> urlBySubject = new HashMap<String,URL>();
	 ...
	 */
    public LocalService() {
    	init(Properties.get("LocalService.path"));
    }
    ...
    private void init(String path) {
    	debugging = Properties.getBool("LocalService.debug");
        		...
        		String[] fields = line.trim().split(":");
         		if (fields.length>0 && !fields[0].startsWith("#") && fields[0].length()>0) {
    		        ...
    		        urlBySubject.put(subject+":"+i,pictureURL);
       		        if (debugging) {
    		        	System.err.println("[debug] picture: \""+subject+":"+i+"\" = "+pictureURL.toString());
    		        }
          		}
         	}
        	in.close();
         } catch(IOException ex) {
			System.err.println("[error] LocalService: can't read index: " + indexURL.toString());
			System.exit(1);
        }
    }
